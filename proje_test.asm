...
.data
	...
	header_hs    		BYTE 	"Highest Score: ",0
	...
	game_action:
		...
		input_loop:
			...
			input_ok:
				...
				; esi kullanicinin girdigi degeri gosteriyor!!!
		...
		all_inputs_ok:
			...
			tries_loop:	
				...
				mov ecx, 4
				mov esi, 0 							; match control flag ( if 4 then we have a match )

				reentry_loop:
					mov al, user_inputs[edx]
					;mov ah, user_inputs[ebx]
					...
					inc esi 						; we have a match, inc flag
					inc edx 
					inc ebx 						
					jmp normal_end
				...
				; if we reached to this point then we have a full match
				inc edi
				pop ecx 							; we jump break_re_loop so we have pop ecx to not mess with ret adress, ebp etc.
